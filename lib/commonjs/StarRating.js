...
const StarRating = _ref => {
  ...
  const swipeStartLocationX = _react.default.useRef(0);
  const hasExceededVerticalThreshold = _react.default.useRef(false);
  const isSwipeActive = _react.default.useRef(false);
  ...
  const panResponder = _react.default.useMemo(() => {
    const calculateRating = function (x) {
      ...
      const newRating = step !== 'full' ? Math.max(0, Math.min(Math.round(x / width.current * maxStars * multiplier + 0.2) / multiplier, maxStars)) : Math.ceil(x / width.current * maxStars);
      return newRating;
    };
    ...
    const setSwipeActive = nextIsActive => {
      if (!enableSwiping) {
        return;
      }
      if (isSwipeActive.current !== nextIsActive) {
        isSwipeActive.current = nextIsActive;
        onSwipeActiveChange === null || onSwipeActiveChange === void 0 ? void 0 : onSwipeActiveChange(nextIsActive);
      }
    };
    ...
    return _reactNative.PanResponder.create({
      ...
      onMoveShouldSetPanResponderCapture: (_, gestureState) => Math.abs(gestureState.dx) > 2 || Math.abs(gestureState.dy) > 2,
      onPanResponderMove: (_, gestureState) => {
        if (enableSwiping) {
          if (!hasExceededVerticalThreshold.current && Math.abs(gestureState.dy) > swipeVerticalThreshold) {
      ...
      onPanResponderEnd: (_, gestureState) => {
        const locationX = swipeStartLocationX.current + gestureState.dx;
        const newRating = hasExceededVerticalThreshold.current ? lastResolvedRating.current : calculateRating(locationX);
        if (!hasExceededVerticalThreshold.current) {
          handleChange(newRating);
        }
  ...
  }, [rating, maxStars, onChange, enableSwiping, swipeVerticalThreshold, onSwipeActiveChange, onRatingStart, onRatingEnd, animationConfig.delay, step, multiplier]);
